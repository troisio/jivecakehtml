export default class OAuthRedirectController {
  constructor($scope, $state, $timeout, $mdSidenav, $q, paymentProfileService, storageService, permissionService, uiService, transactionService, JiveCakeLocalStorage, SearchEntity) {
    this.$scope = $scope;
    this.$state = $state;
    ...
    if (typeof error === 'undefined' || error === null) {
      ...
      this.permissionService.search(auth.idToken, {
        ...
        transactionFuture.then(transactionSearch => {
          ...
          this.uiService.notify('Sorry, we were unable to load your data');
          this.$state.go('application.public.home');
        });
      }, () => {
        this.uiService.notify('Sorry, we were unable to load your organizations');
        this.$state.go('application.public.home');
      });
    } else {
      this.uiService.notify('Sorry, we were unable to get your data from Auth0');
      this.$state.go('application.public.home');
    }
  }
}
